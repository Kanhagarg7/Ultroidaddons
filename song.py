...
@kanha_cmd(pattern="song ?(.*)")
async def _(event):
    kanha_bot = event.client
    try:
        await kanha_bot(ImportChatInviteRequest("DdR2SUvJPBouSW4QlbJU4g"))
    ...
    try:
        async for event in kanha_bot.iter_messages(
            chat, search=args, limit=1, filter=filtermus
        ):
            await kanha_bot.send_file(current_chat, event, caption=event.message)
        await okla.delete()
    except Exception:
